...
double evolve(BasePoints &base, const vector<double> &points, vector<double> &b, vector<int> &where_point, int &cnt_itrs, double last_diff)
{
    ...
    cnt_itrs = solveLinearSystemItr(A, b, base.x, series_size, series_size, max(EPSILON, last_diff * 0.8));
...
int main(int argc, char *argv[])
{
    ...
    for (double diff = EPSILON + 1.; diff > EPSILON;)
    {
        ...
        int cnt_itrs;
        diff = evolve(base, points, b, where_point, cnt_itrs, diff);
