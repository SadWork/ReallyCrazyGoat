...
namespace dev_func
{
    ...
    bool NextSetNoRepeat(vector<Integer> &a, int n, int m)
    {
        int k = m;
        for (int i = k - 1; i >= 0; --i)
            if (a[i] < n - k + i + 1)
    ...
    template <typename Integer>
    bool NextSet(vector<Integer> &a, int n, int m)
    {
        int j = m - 1;
        while (a[j] == n && j >= 0)
            j--;
    ...
    // Генерирует по порядку все выборы m1 + ... + mn = sum в алфавитном порядке
    template <typename Integer>
    bool NextMultiCoeff(vector<Integer> &a, int n, int sum)
    {
        a[0]++;
        for (int i = 1; i < n; i++)
        {
        ...
        bool ans = NextSetNoRepeat(a, sum + n - 1, n - 1);
